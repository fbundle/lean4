...
public def extrinsicFix_eq_wellFoundedFix [∀ x, Nonempty (C x)] {F : (x : α) → ((y : α) → C y) → C x}
    (r : α → α → Prop) (F' : ∀ x, (∀ y, r y x → C y) → C x)
    (wf : WellFounded r) (heq : ∀ x G, F x G = F' x (fun x _ => G x)) {x : α} :
    extrinsicFix F x = wf.fix F' x := by
  have : TerminatesTotally F := ⟨r, F', wf, heq⟩
  simp only [extrinsicFix, dif_pos this]
  induction x using wf.induction
  rename_i x hx
...
public noncomputable def wfFix_induction {r : α → α → Prop} {wf : WellFounded r}
    {F : ∀ x, (∀ y, r y x → C y) → C x}
    {motive : ∀ x, C x → Sort _}
    (h : ∀ x, (∀ y, r y x → motive y (wf.fix F y)) → motive x (wf.fix F x))
    (x : α) :
    motive x (wf.fix F x) := by
  exact WellFounded.recursion (C := fun x => motive x (wf.fix F x)) wf x h
